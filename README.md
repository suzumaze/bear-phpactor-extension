...
| Twig / Qiq embedded-template definition jump | In a standard Twig template, cursor on `{{ rel }}`; in a standard Qiq template, cursor on `{{= $this->rel }}` or `{{h $this->rel }}` → jumps to the same-engine template for the `app://self/...` resource declared by the parent Resource's `#[Embed(rel: 'rel', src: ...)]` |
...
|---|---|---|
| `var/templates/{App,Page}/.../*.html.twig` | a complete bare Twig print expression: `{{ rel }}` | `var/templates/` template for `#[Embed(rel: 'rel', src: 'app://self/...')]` |
| `var/qiq/template/{App,Page}/.../*.php` | `{{= $this->rel }}` or `{{h $this->rel }}` | `var/qiq/template/` template for the same `#[Embed]` relation |

Both the parent and embedded Resource and the target template must exist inside the project. Only named string `rel:` and `src:` arguments on `#[Embed]`, with `app://self/...`, are used. A repeated `rel` resolves only when every occurrence names the same URI; otherwise it returns no location. General Twig `include`/`extends`/`import`, Twig property or filter expressions, Qiq code tags and arbitrary PHP expressions, imported-app resources, custom template roots, and ambiguous conventions are intentionally unsupported.
