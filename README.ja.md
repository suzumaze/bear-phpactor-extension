...
| Twig / Qiqテンプレート定義ジャンプ | 静的なテンプレート名にカーソルを置く → `extends`・`include`・`block()`・`render()`・`setLayout()`などの参照先へ飛ぶ。さらに、`{{ rel }}` / `{{= $rel }}`などの埋め込みrelationから、親Resourceの`#[Embed]`が宣言する同じテンプレートエンジン用テンプレートへ飛ぶ |
...
明示的なテンプレートパスと、BEAR.Sundayの`#[Embed]`に対応するrelationの2種類を扱います。対応する標準配置とカーソル位置は意図的に限定しています。

| 種類 | 対応するカーソル位置 | 解決先 |
|---|---|---|
| Twigの明示パス | `extends`・`include`・`include()`の第1引数、`block()`の第2引数にある静的文字列 | `src/Resource`、次に`var/templates`から見つかるテンプレート |
| Qiqの明示パス | Qiqヘルパー構文またはネイティブPHPの`setLayout()`・`render()`・`extends()`にある静的文字列 | `var/qiq/template`内の`.php`テンプレート。`./`・`../`は現在のテンプレートを基準に解決 |
| TwigのEmbed relation | `var/templates/{App,Page}/.../*.html.twig`にある`{{ rel }}`、`{{ rel|raw }}`の先頭relation | 対応する`#[Embed]`関係の`var/templates/`内テンプレート |
| QiqのEmbed relation | `var/qiq/template/{App,Page}/.../*.php`にある`{{= $rel }}`、`{{h $rel }}`。旧`$this->rel`も対応 | 同じ`#[Embed]`関係に対応する`var/qiq/template/`内テンプレート |

Embed relationでは、親・埋め込み先のResourceと解決先テンプレートが、すべてプロジェクト内に実在する必要があります。`#[Embed]`の名前付き文字列引数`rel:`と`src:`だけを解釈します。絶対形式の`app://self/...` / `page://self/...`と相対形式の`/...`に対応し、相対形式は親Resourceのschemeを引き継いで`self` hostとして解決します。同じ`rel`が繰り返される場合、正規化後のURIがすべて同じときだけ解決し、異なるURIなら場所を返しません。動的なテンプレート式、Twigの`import`とプロパティ式、既知のヘルパー以外のQiq/PHP式、ImportAppのリソース、カスタムテンプレートルート、曖昧な規約は意図的に未対応です。
...
- **テンプレートジャンプは、BEARモジュールの標準ローダー設定だけに従います。** Twigは`Madapaja.TwigModule`の標準`AppPathProvider`と同じく`src/Resource`、次に`var/templates`を検索します。Qiqは`BEAR.QiqModule`と同じく`var/qiq/template`を`.php`拡張子で検索します。DI束縛からは確定できないTwigの独自パス・名前空間と、Qiqの独自パス・拡張子・collectionは解決しません。明示的なテンプレートパスは静的な文字列リテラルだけが対象です。
- **公式のVS Code用phpactorクライアントは、Twig文書をphpactorへ送りません。** 現在のdocument selectorは`php`と`blade`だけなので、このパッケージのTwigジャンプを使うには、phpactorをTwig文書にも接続するLSPクライアント設定が必要です。BEAR標準配置のQiqファイルは`.php`なので、通常どおりphpactorへ届きます。
